...
#        col_std=statistics.stdev(col_values)                  # Another Way
        col_std=math.sqrt(sum([(X_df[j][i]-col_mean)**2 for j in range(len(X_df))])/(len(X_df)-1))

        for k in range(len(x)):
            x[k][i]= (x[k][i]-col_mean) / col_std
