...
lamb = 0.01       #                                  #  for Hyper Parameter for regularization
#------------------------------------------------------#
#======================================================#
...
def deriv_fun(x, y, theta, i3):
    ...
    for i2 in range(len(x)):
        z += (hypothesis_fun(x, theta, i2) - y[i2]) * x[i2][i3]    # here added ridge regularization
    return z
...
#---------------------------------------------------------------main function---------------------------------#
def main():
    def get_theta(theta, x, y, alpha, iteration, tol,lamb):

        for itr in range(iteration):
            new_theta = [0] * len(theta)

            for i3 in range(len(theta)):
                new_theta[i3] = theta[i3] - alpha * (deriv_fun(x, y, theta, i3) + (2*lamb*theta[i3]))
...
#            print(f'Iteration {itr}: theta = {theta}, cost = {j_theta}, diff = {diff}')
    ...
    result = get_theta(theta, x, y, alpha, iteration, tol,lamb)
    print(len(x_test),len(x_test[0]),len(theta))
